...
# skip() -- an HONEST non-pass, non-fail marker (never silently omitted, never counted as fail).
# #2500 X2: the two nested FUNCTIONAL callers (wt-strip AC-C2 / home-absolute AC-6) set
# LEDGER_SMOKE_1851_BENCH_OFF=1 to skip ONLY the #1851 AC2(i)/(ii) timing-benchmark arms below
# (the ~5.5-minute re-paid cost, #2500 E5) -- every OTHER assertion in this file still runs.
# Default (unset, standalone/CI invocation): full power retained, arms run exactly as before.
skip() { echo "SKIP: $1"; }
...
# M10. INVALID-EFFORT lint both-ends (#1503 -- the EFFORT lever's sibling to M7's MODEL lint). METYPO
# carries an off-vocabulary effort ('turbo'); MCFG (already defined above) carries the on-vocabulary
# 'medium' as the valid-value control -- same fixture M7's GREEN arm reuses for the model lever.
METYPO="$TMP/metypo.env"; printf 'CC_ROLE_EXECUTOR_MODEL=sonnet\nCC_ROLE_EXECUTOR_EFFORT=turbo\n' > "$METYPO"
OUT=$(CC_ROLES_ENV="$METYPO" node "$LED" resolve-role-model --role executor --with-effort 2>"$TMP/elint.err")
{ [ "$OUT" = "sonnet xhigh" ] && [ "$(command grep -Ec 'INVALID-EFFORT' "$TMP/elint.err"; true)" -ge 1 ]; } \
  && ok "M10 RED->GREEN: typo 'turbo' -> resolve prints sonnet xhigh + INVALID-EFFORT on stderr" || bad "M10 typo effort should print sonnet xhigh + INVALID-EFFORT (out=$OUT err=$(cat "$TMP/elint.err"))"
OUT=$(CC_ROLES_ENV="$MCFG" node "$LED" resolve-role-model --role executor --with-effort 2>"$TMP/elint2.err")
{ [ "$OUT" = "sonnet medium" ] && [ "$(command grep -Ec 'INVALID-EFFORT' "$TMP/elint2.err"; true)" -eq 0 ]; } \
  && ok "M10 GREEN: valid effort 'medium' -> resolve prints sonnet medium + NO INVALID-EFFORT" || bad "M10 valid effort should print sonnet medium + no INVALID-EFFORT (out=$OUT err=$(cat "$TMP/elint2.err"))"
...
#
# #2500 X2 nested-mode knob: this whole AC2(i)/(ii) block (~5.5 of the ~7 min this file otherwise takes,
# E5) is the ONLY part of the #1851 section that is expensive -- AC1/AC3-AC8 above/below are all
# sub-second. guard-smoke-suite.sh's two nested FUNCTIONAL callers (the wt-strip AC-C2 loop and the
# home-absolute AC-6 loop) only assert "exits 0, no FAIL: line" on this file when they invoke it as a
# regression control -- they never read its individual PASS labels -- so they set
# LEDGER_SMOKE_1851_BENCH_OFF=1 to skip re-paying this benchmark. A STANDALONE invocation (this file's
# own guard-smoke-suite.sh manifest entry, or a direct `bash hooks/3role-ledger-smoke-test.sh`) leaves
# the knob unset, so the arms -- and their red-arm power (NR1) -- run exactly as before.
if [ "${LEDGER_SMOKE_1851_BENCH_OFF:-}" = "1" ]; then
  skip "#1851 AC2(i)/(ii): benchmark arms skipped (LEDGER_SMOKE_1851_BENCH_OFF=1 -- nested functional-control caller, #2500 X2); run this file standalone (knob unset) for the timing arms + their red-arm power"
else
